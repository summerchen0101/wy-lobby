...
function compareGamesByLobbySort(a: Game, b: Game, menu: LobbyGameSortMenu): number {
 ...
 */
export function lobbyThirdPartyRowToApiGame(row: LobbyThirdPartyRow): Game | null {
  const platform =
...
function lobbyWalletTypeFromWire(raw: unknown): LobbyWalletType | undefined {
  ...
  if (raw === 'UNKNOWN_WALLET_TYPE' || raw === 0 || raw === '0') return 'UNKNOWN'
  return undefined
}
